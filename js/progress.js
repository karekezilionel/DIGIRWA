const ProgressManager = {
  ...
  completeLesson: function(courseId, lessonId) {
    ...
    if (!progress[courseId]) {
      progress[courseId] = { completedLessons: [], quizScore: null };
    }
  ...
  isCourseUnlocked: function(courseId) {
    ...
    const prevCourseId = courseId - 1;
    const prevProgress = progress[prevCourseId];
    
    if (!prevProgress) return false;
    
    
    return prevProgress.quizScore !== null && prevProgress.quizScore >= 50;
  },
  ...
  getCourseCompletionPercentage: function(courseId, totalLessons) {
    ...
    const completed = progress[courseId].completedLessons ? progress[courseId].completedLessons.length : 0;
